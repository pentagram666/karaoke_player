...
function scrollIntoViewIfNeeded(el) {
	...
	else if (elBottom > ctBottom) container.scrollTop = elBottom - container.clientHeight + 8;
}
...
function handleAction(action, el) {
	switch (action) {
		case 'open-songs': showScreen('screen-songs'); break;
		case 'open-upload': document.getElementById('inputfile').click(); break;
		case 'open-upload-screen': showScreen('screen-upload'); break;
		case 'open-themes': showScreen('screen-themes'); break;
		case 'cat-prev': if (typeof switchCategory === 'function') switchCategory(-1); break;
		case 'cat-next': if (typeof switchCategory === 'function') switchCategory(1); break;
		case 'back': showScreen('screen-home'); break;
		case 'back-home':
		...
		case 'play-pause':
			if (typeof player !== 'undefined' && player) {
				if (player.state === player.PLAYING) { player.pause(); updateHudState(false); }
				else { player.play(); updateHudState(true); }
			}
			break;
		case 'stop':
			if (typeof player !== 'undefined' && player) { player.stop(); updateHudState(false); }
			break;
		case 'next-song': playAdjacentSong(1); break;
		case 'prev-song': playAdjacentSong(-1); break;
	}
}
...
function applyTheme(theme) {
	...
	for (var i = 0; i < cards.length; i++) {
		...
		if (cards[i].getAttribute('data-theme') === theme) cards[i].classList.add('selected');
	}
}
...
function resetControlsTimer() {
	...
	controlsHideTimer = setTimeout(function() {
		if (currentScreen === 'screen-player') hidePlayerControls();
	}, 4000);
}
...
function initUI() {
	...
	var themes = ['#sunset', '#basic', '#christmas', '#none'];
	currentTheme = themes.indexOf(hash) >= 0 ? hash.substring(1) : (window.defaultTheme || 'sunset');
	...
	document.addEventListener('keydown', function(e) {
		...
		if (currentScreen === 'screen-player') {
			...
			if (key === ' ' || key === 'MediaPlayPause' || key === 179) {
				e.preventDefault(); handleAction('play-pause'); return;
			}
			if (key === 'MediaStop' || key === 178) {
				e.preventDefault(); handleAction('stop'); return;
			}
			if (key === 'MediaTrackNext' || key === 176) {
				e.preventDefault(); handleAction('next-song'); return;
			}
			if (key === 'MediaTrackPrevious' || key === 177) {
				e.preventDefault(); handleAction('prev-song'); return;
			}

			var digit = (typeof key === 'string' && key.length === 1 && key >= '0' && key <= '9') ? key
			           : (typeof key === 'number' && key >= 48 && key <= 57) ? String.fromCharCode(key) : null;
			if (digit !== null) {
				e.preventDefault();
				if (typeof handleQueueNumberKey === 'function') handleQueueNumberKey(digit);
				return;
			}

			if ((key === 'Enter' || key === 13) && typeof queueDigits !== 'undefined' && queueDigits !== '') {
				e.preventDefault();
				if (typeof commitQueueNumber === 'function') commitQueueNumber();
				return;
			}

			if (key === 'Backspace' && typeof queueDigits !== 'undefined' && queueDigits !== '') {
				e.preventDefault();
				if (typeof backspaceQueueNumber === 'function') backspaceQueueNumber();
				return;
			}

			if (key === 'Escape' || key === 'GoBack' || key === 'BrowserBack' || key === 10009) {
				e.preventDefault();
				if (typeof queueDigits !== 'undefined' && queueDigits !== '') {
					if (typeof clearQueueNumberPanel === 'function') clearQueueNumberPanel();
				} else if (typeof queueCountdownActive !== 'undefined' && queueCountdownActive) {
		...
		if (key === 'ArrowDown' || key === 40) {
			...
			e.preventDefault();
			if (currentScreen === 'screen-songs') { if (typeof switchCategory === 'function') switchCategory(1); }
			else if (currentScreen !== 'screen-player') { moveFocus(1); }
	...
	document.getElementById('screen-player').addEventListener('click', function(e) {
		if (!e.target.closest('.ctrl-btn') && !e.target.closest('.player-hud')) showPlayerControls();
	});
